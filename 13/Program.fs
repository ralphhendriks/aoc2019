...
type Tile = | Empty = 0 | Wall = 1 | Block = 2 | Paddle = 3 | Ball = 4
...
let main _ =
    ...
    let answer1 =
        match (initialize program) with
        | { Computer.State = Finished; Computer.Outputs = outputs; } -> List.rev outputs |> List.map int
        | _ -> failwith "Computer waiting for input, not expected here."
        |> List.chunkBySize 3
        |> List.filter (fun outputInstruction -> outputInstruction.[2] = 2)
        |> List.length
    printfn "Answer 1: %i" answer1

    let updateTileMap tileMap computerOutputs =
        computerOutputs
        |> List.map int
        |> List.rev
        |> List.chunkBySize 3
        |> List.map (fun l -> (l.[0], l.[1]), l.[2])
        |> List.fold (fun m t -> Map.add (fst t) (snd t) m) tileMap

    let rec play computer prevTileMap =
        let tileMap = computer.Outputs |> updateTileMap prevTileMap
        let hiScore = tileMap.[(-1, 0)]
        let numBlocks = tileMap |> Map.filter (fun _ v -> v = 2) |> Map.count
        let paddle = tileMap |> Map.filter (fun _ v -> v = 3) |> Map.toSeq |> Seq.exactlyOne |> fst
        let ball = tileMap |> Map.filter (fun _ v -> v = 4) |> Map.toSeq |> Seq.exactlyOne |> fst
        if numBlocks < 1 then hiScore
        else
            let joystick =
                match (fst ball) with
                | x when x < (fst paddle) -> -1L
                | x when x > (fst paddle) -> 1L
                | _ -> 0L
            let nextComputer = input joystick {computer with Outputs = list.Empty}
            play nextComputer tileMap

    let solvePart2 (program: int64 array) =
        program.[0] <- 2L
        let computer = initialize program
        play computer Map.empty

    solvePart2 (Array.copy program) |> printfn "Answer 2: %i"
